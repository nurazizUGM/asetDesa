...
class Aset extends CI_Controller
{
	...
	$this->form_validation->set_rules(
		'kode_aset',
		'Kode Aset',
		'required|trim|is_unique[aset.kode_aset]',
		array(
	...
	if ($this->form_validation->run() == TRUE) {
		...
		// Pastikan ada request untuk generate QR Code
		if ($generate) {
			...
			$data = array(
				...
				'total_harga' => ($this->input->post('volume') * $this->input->post('harga')),
	...
	public function ubahAset()
	{
		...
		if ($this->form_validation->run() != FALSE) {
			...
			if ($generate) {
				...
				$url = 'http://asetdesakarangtengah.my.id/aset/detail/' . $id_aset;
	...
	public function hapusAset($id_aset)
	{
		...
		$this->db->where('id_aset', $id_aset);
		$get_image_file = $this->db->get('aset')->row();
		@unlink('src/img/qrcode/' . $get_image_file->qr_code);
